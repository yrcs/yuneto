...
var ProviderSet = wire.NewSet(NewDB, NewData, NewTransaction, NewDictRepo)
...
func NewDB(conf *conf.Data, logger log.Logger) *gorm.DB {
	log := log.NewHelper(log.With(logger, "module", "datadict/data/db"))
	...
	if err = db.Set("gorm:table_options", "ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='数据字典表'").AutoMigrate(&po.Dict{}); err != nil {
		log.Fatal(err)
	}
...
func NewData(db *gorm.DB, logger log.Logger) (*Data, func(), error) {
	l := log.NewHelper(log.With(logger, "module", "datadict/data/data"))
	d := &Data{
		db:  db,
		log: l,
	}
